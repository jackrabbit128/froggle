...
public final class Board {
  ...
  public Board(int width, int height, Collection<Die> dice) {
    ...
    _width = width;
    _height = height;
    _dice = List.copyOf(dice);
  }

  public void shuffle(Random random) {
    var sortedDice = new ArrayList<>(_dice);
    sortedDice.sort(Comparator.comparing(System::identityHashCode));

    var shuffledDice = new ArrayList<Die>(_dice.size());
    var indices = IntStream.range(0, sortedDice.size()).boxed().collect(Collectors.toList());
    while (!indices.isEmpty()) {
      var index = indices.remove(random.nextInt(indices.size()));
      shuffledDice.add(sortedDice.get(index));
    }
